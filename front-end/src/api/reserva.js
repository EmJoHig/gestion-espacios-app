...
export const getReservasFilterRequest = async (token, filtros) => {

  const queryParams = new URLSearchParams(filtros).toString();

  try {
    const response = await axios.get(`${API_URL}/reserva/get_reservas_filter?${queryParams}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
    ...
    console.error('Error fetching getReservasFilterRequest:', error);
    throw error;
  }
};


export const bajaReservaRequest = async (token, body) => {

  try {
    const response = await axios.post(`${API_URL}/reserva/dar_baja_reserva`, body, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
    ...
    console.error('Error bajaReservaRequest: ', error);
    throw error;
  }
}
